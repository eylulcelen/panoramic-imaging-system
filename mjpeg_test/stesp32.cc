/* https://docs.opencv.org/3.4/d1/d46/group__stitching.html */
...
void update_texture_thread(VideoStitcher *st) {
    while (running) {
...
int main() {
    ...
    std::vector<std::string> capture_urls {
        "http://10.97.66.238/image",
        "http://10.97.66.80/image"
    };

    VideoStitcher *st;
    try {
        st = new VideoStitcher(capture_urls);
    } catch (std::exception e) {
        return -1;
    }

    std::thread thread(update_texture_thread, st);
...
int main(int argc, char** argv) {
    ...
    VideoStitcher st(capture_urls);
    if (st.setup()) {
        std::cerr << "Stitcher setup failed\n";
        return 1;
    }
